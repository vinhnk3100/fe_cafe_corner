  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {cafes.map((cafe: Cafe) => (
        <Card
          key={cafe.id}
          className="group w-full max-w-sm border shadow-lg ease-in-out duration-300 border-buttonColor overflow-hidden hover:border-buttonHoverColor"
        >
              ...
              <div className="flex flex-row items-center gap-4">
                <div className="flex flex-row items-center gap-1">
                  ...
                  <span className="text-md font-medium text-gray-200 group-hover:text-white">
                    {Math.floor(
                      (cafe.totalLike / (cafe.totalLike + cafe.totalDislike)) *
                        100
                    )}
                    %
                  </span>
